...
def parse_args():
    ...
    parser.add_argument(
        "--hduname", nargs="+", metavar="idn", help="HDU names to print"
    )
    parser.add_argument(
        "--hduindex", nargs="+", type=int, metavar="idx", help="HDU ids to print"
    )
    parser.add_argument("--all", action="store_true", help="print all extensions")
...
def imhead():
    ...
    for ffile in optlist.fitsfile:
        ...
        if optlist.info:  # just print the image info and exit
            ...
            hduids = iu.get_requested_hduids(hdulist, optlist.hduname, optlist.hduindex)
            for hduid in hduids:
                print("#--------{}:{}--------".format(hduid, hdulist[hduid].name))
