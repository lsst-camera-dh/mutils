...
def parse_args():
    ...
    xgroup.add_argument('--sharex', dest='sharex', action='store_true',
                        help="Share X-axis range, this is default")
    xgroup.add_argument('--no-sharex', dest='sharex', action='store_false',
    ...
    parser.add_argument("--style", default='ggplot', required=False,
                        choices=style_list, help="default: %(default)s")
...
def main():
    ...
    for findex in range(0, nfiles):
        ...
        logging.debug('using title_nchars=%d title_fontsize=%s',
                      title_nchars, title_fontsize)
        if optlist.overlay:
        ...
        if not optlist.offset:
            ...
            ax.set_ylabel(
                'signal - mean + 5*j*stdev, j=0,1,..', size='x-small')
        else:
    ...
    if optlist.title:  # set the suptitle
        fig.set_tight_layout({'h_pad': 0.50, 'w_pad': 1.0,
                              'rect': [0, 0, 1, 0.97]})
    else:
        fig.set_tight_layout({'h_pad': 0.50, 'w_pad': 1.0,
                              'rect': [0, 0, 1, 1]})
