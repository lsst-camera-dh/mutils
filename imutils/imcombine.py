...
def parse_args():
    ...
    # processing options
    #
    parser.add_argument("--ifile", nargs=1, type=str,
                        help="output fits_image name")
    parser.add_argument("--result", nargs=1, type=str,
                        help="output fits_image name")
    # processing choices for image combining
    mgroup = parser.add_mutually_exclusive_group()
    mgroup.add_argument("--median", action='store_true',
                        help="use median to combine images")
    mgroup.add_argument("--mean", action='store_true',
                        help="use mean to combine images")
    mgroup.add_argument("--sigmaclipped", const=3.0,
                        metavar='nsig', nargs='?',
                        help="use sigmaclipped mean to combine images")
    mgroup.add_argument("--rank", const=50,
                        metavar='percentile', nargs='?',
                        help="use percentile, eg. median=50")
    # hdu selection
    hgroup = parser.add_mutually_exclusive_group()
    hgroup.add_argument("--hduname", nargs='+', default=[],
                        metavar='idn', help="list of HDU names")
    hgroup.add_argument("--hduindex", nargs='+', type=int, default=[],
                        metavar='idx', help="list of HDU ids")
    # limit output to an ROI
    parser.add_argument("--region", default=None,
                        help="ROI region fmt: \"x1:x2, y1:y2\"")
    # bias subtraction controls
    parser.add_argument("--bias_image", nargs=1,
                        help="subtract a bias image")
...
def main():
    ...
    for hduid, hdui in enumerate(iimages[0]):
        ...
        if hduid in hduids_to_comb:
            ...
            iu.image_combine_hdu(iimages, hduid, method, region, bimage,
                                 optlist.bias, optlist.btype, scaling, hduo)
            # finish up this hdu
            hduo.update_header()
        ...
        if hduid in hduids_to_proc:
            if not isinstance(hdui, (fits.ImageHDU,
