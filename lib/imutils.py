...
def init_image_hdu(hdui: fits.ImageHDU, hdulisto: fits.HDUList,
...
def get_requested_hduids(hdulist: fits.HDUList,
...
def subtract_bias(bstring: str, btype: str, hdu: fits.ImageHDU):
    ...
    if btype in ('byrow', 'byrowsmooth', 'byrowcol', 'byrowcolsmooth'):
        ...
        if re.search(r'smooth', btype):  # smooth the row bias vector a bit
            logging.debug('smoothing ser overscan with Gaussian1DKernel')
            ...
            for hpar, bpar in zip(hdu_pars, base_pars):
                for key in bpar.keys():
                    if hpar[key] != bpar[key]:
                        logging.error('file parameter mismatch: %s: %s != %s',
                                      key, hpar[key], bpar[key])
                        sys.exit(1)
...
def image_combine_hdu(iimages: list, hduid: int, method: list, region: tuple,
                      bimage: fits.HDUList, bstring: str,
                      btype: str, scaling: tuple, hduo: fits.ImageHDU):
    """
    ...
    if re.match(r"mea", method[0]):
        ...
        hduo.data = np.nanmean(stats.sigma_clip(np.array(hdudata_list),
                                                method[1], axis=0,
                                                masked=False), axis=0)
