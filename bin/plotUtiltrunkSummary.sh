...
if [ $doecho ] ; then
    echo trender ${st} ${sv} --dur ${duration} ${timebins} --title "Utiltrunk Thermal Summary" --plot --layout 4x2 --outside --overlayregex -- "${regexes[@]}"
    exit
fi

trender --dpi 300 ${st} ${sv} --dur ${duration} ${timebins} --title "Utiltrunk Thermal Summary" --plot --layout 4x2 --outside --overlayregex -- "${regexes[@]}"
